use crate::docs::{get_method_doc, get_property_doc};
use crate::document::DocumentState;
use crate::import_resolver::{load_imported_macros, resolve_import_path};
...
pub fn get_hover(doc: &DocumentState, position: Position, current_uri: &str) -> Option<Hover> {
    ...
    info!(
        "Hover word: '{}' at position {}:{}",
        word, position.line, position.character
    );
    ...
    if let Some(macro_ref) = word.strip_prefix('@') {
        ...
        if let Some(dot_pos) = macro_ref.find('.') {
                        ...
                        if let Ok(imported_macros) = load_imported_macros(&resolved_path) {
                            ...
                            for (name, _span, body) in imported_macros {
                                if name == macro_name {
                                    ...
                                    let alias_display =
                                        import_info.alias.as_ref().unwrap_or(&import_info.path);
            ...
            return Some(create_hover(&format!(
                "**Macro:** `@{}:{}`\n\nNamespaced macro (not resolved)",
                namespace, macro_name
            )));
        }
...
fn format_macro_body(body: &[githook_syntax::Statement]) -> String {
    ...
    for stmt in body {
        let line = match stmt {
            ...
            githook_syntax::Statement::MacroCall {
                name, namespace, ..
            } => {
...
fn get_method_hover(method: &str) -> Option<String> {
    if let Some(doc) = get_method_doc(method) {
        return Some(format!(
            "**{}**\n\n{}\n\n**Example:**\n```githook\n{}\n```",
            doc.name, doc.description, doc.example
        ));
    }
    None
}
