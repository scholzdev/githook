        ...
        Ok(latest) => {
            ...
            if is_newer_version(&latest, current_version) {
                ...
                let current_exe = env::current_exe()
                    .context("Failed to locate running executable")?;
                
                fs::copy(&current_exe, &backup_path)
                    .context("Failed to create backup")?;
                
                println!("{}  Created backup at: {}", "-".dimmed(), backup_path.display());
...
fn get_latest_version() -> Result<String> {
    ...
    Err(anyhow::anyhow!("Could not parse latest version from GitHub"))
}
...
fn install_update(version: &str) -> Result<()> {
    ...
    fs::write(temp_file.path(), &binary_data)
