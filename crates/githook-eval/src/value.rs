...
/// A dynamically-typed value in the GitHook runtime.
///
/// The value system supports common data types and rich object types
/// with methods that can be called from GitHook scripts.
///
/// # Type Hierarchy
///
/// - **Primitive Types**: String, Number, Bool, Null
/// - **Collections**: Array (ordered list of values)
/// - **Objects**: Complex types with properties and callable methods
///
/// # Example
///
/// ```rust
/// use githook_eval::Value;
///
/// let string_val = Value::String("hello".to_string());
/// let number_val = Value::Number(42.0);
/// let bool_val = Value::Bool(true);
/// let array_val = Value::Array(vec![number_val, bool_val]);
/// ```
#[derive(Debug, Clone)]
...
pub enum Value {
    ...
    Bool(bool),
    /// The null/nil value representing absence of data
    Null,

    /// An ordered collection of values
    Array(Vec<Value>),
    /// A complex object with properties and optional context
    Object(Object),
}

/// A complex object with properties and optional typed contexts.
///
/// Objects in GitHook can represent various domain concepts like files,
/// Git repositories, HTTP responses, etc. Each object type can have
/// associated methods that are callable from scripts.
#[derive(Debug, Clone)]
pub struct Object {
    /// The type name of this object (e.g., "File", "Git", "String")
    pub type_name: String,
    /// Key-value properties accessible via property access syntax
    pub properties: HashMap<String, Value>,

    // Context objects for different domain types
    /// File-specific context and methods
    pub file_context: Option<FileContext>,
    /// Path-specific context and methods
    pub path_context: Option<PathContext>,
    /// Git repository context and methods
    pub git_context: Option<GitContext>,
    /// Collection of files (staged, modified, etc.)
...
impl Object {
    /// Create a new empty object with the given type name.
    ///
    /// # Example
    ///
    /// ```rust
    /// use githook_eval::value::Object;
    ///
    /// let obj = Object::new("MyType");
    /// assert_eq!(obj.type_name, "MyType");
    /// ```
    ...
    /// Add a property to this object (builder pattern).
    ///
    /// # Example
    ///
    /// ```rust
    /// use githook_eval::value::{Object, Value};
    ///
    /// let obj = Object::new("Person")
...
impl Value {
    ...
    pub fn call_method(&self, name: &str, args: &[Value]) -> Result<Value> {
        match self {
            Value::String(s) => {
                let ctx = StringContext::new(s.clone());
                let string_args: Result<Vec<&str>> = args
                    .iter()
                    .map(|v| v.as_string().map(|s| Box::leak(s.into_boxed_str()) as &str))
                    .collect();
    ...
    fn call_object_method(&self, obj: &Object, name: &str, args: &[Value]) -> Result<Value> {
        if obj.type_name == "File" {
            if let Some(ctx) = &obj.file_context {
                let string_args: Result<Vec<String>> = args.iter().map(|v| v.as_string()).collect();
